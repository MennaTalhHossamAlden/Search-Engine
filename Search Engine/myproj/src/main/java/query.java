...
//                if(br==null) break;
    ...
    public void process() {
        ...
        //remove each stop words and steaming words
        String pharse_search="";
        boolean find_pharse_search=false;
        for (int i = 0; i < temp_query.length; i++) {

//            if(index!=-1){
//                 if(!find_pharse_search){
//                     find_pharse_search=true;
//                     pharse_search=temp_query[i].substring(index,temp_query[i].length())+" ";
//                 }
//                 else {
//                     pharse_search+=temp_query[i].substring(0,index+1);
//                     find_pharse_search=false;
//                     //// send to phrase search
//                     List<Document>phase_temp=new ArrayList<Document>();
//                     phase_temp=phase_search(pharse_search);
//                     for (Document d:phase_temp) {
//
//                     }
//
//                 }
//            }
//            else if(find_pharse_search){
//                pharse_search+=temp_query[i].substring(0,index+1)+" ";
//            }
...
//            Connection con2 = Jsoup.connect(set.getKey());
//            try {
//                org.jsoup.nodes.Document doc2 = con2.get();
...
//        doc_list.add((Document)Search_Index.find(eq("word", "Page")).first());
//        doc_list.add((Document)Search_Index.find(eq("word", "BBC")).first());
//        doc_list.add((Document)Search_Index.find(eq("word", "Homepage")).first());
...
//        for (int i = 0; i < col2.countDocuments(); i++) {
//            org.bson.Document d = (org.bson.Document) col2.find().sort(new org.bson.Document ("Rank",-1)); // may 2 threads catch the same url ////
//            col2.deleteOne(d);
//            col2.insertOne(d);
//
//        }
...
//                i--;
        ...
        page+=  "    <div class=\"engine_inside  \">\n" +
                ...
                "                <input list=\"search\" id=\"my-input\" name=\"search\" placeholder= \"Try Again\">" +
                "  <datalist id=\"search\">";
